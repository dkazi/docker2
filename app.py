...
# API key από .env (docker-compose το φορτώνει αυτόματα)
...
def save_history(session_name, messages):
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"session": session_name, "messages": messages},
...
def run_detection():
    """Τρέχει GOD_OF_DETECTION incremental μέσω subprocess."""
    env = os.environ.copy()
...
def ask_god_of_chat(question, history):
    """
    Περνάει την ερώτηση στο GOD_OF_CHAT.ask() μαζί με τα detection results.
    Επιστρέφει string απάντηση.
    """
    import GOD_OF_CHAT as god_chat
    results = load_detection_results()
    if not results:
        return ("Δεν υπάρχουν detection results ακόμα. "
                "Βεβαιώσου ότι έχεις ξεκινήσει monitoring και υπάρχουν logs.")
    chat_history = [
...
def generate_pdf(messages, session_name):
    lines = ["LogGuard SOC Export", f"Session: {session_name}", ""]
    for m in messages:
        role = "You" if m["role"] == "user" else "SOC AI"
        lines.append(f"[{role}]")
...
def bar_chart(data, title, color="#4da6ff"):
    ...
    for bar, val in zip(bars, vals):
        ax.text(bar.get_x() + bar.get_width() / 2,
                ...
                ha="center", va="bottom", color=DARK_FG, fontsize=7)
    fig.tight_layout()
    return fig
...
def pie_chart(data, title):
    ...
    ax.legend(wedges, labels, loc="lower center",
              ...
              fontsize=7, frameon=False, labelcolor=DARK_FG)
        ...
        if st.button("✅ Start Monitoring", type="primary",
            ...
            if selected_files:
                ...
                with open(MASTER_FILE_PATH, "w", encoding="utf-8") as f:
                    f.write(
                        f"--- SESSION START: {time.strftime('%H:%M:%S')} ---\n")
                st.rerun()
            else:
...
# ── TABS ───────────────────────────────────────────────────────────────────────
tab_soc, tab_dash, tab_live, tab_hist, tab_flags = st.tabs([
    "🛡️ SOC Analysis",
    "📊 Dashboard",
    "📡 Live Logs",
    "🕓 History",
...
# ══════════════════════════════════════════════════════════════════════════════
#  TAB 1 — SOC ANALYSIS
#  Ροή: prompt → GOD_OF_DETECTION → GOD_OF_CHAT.ask() → απάντηση
# ══════════════════════════════════════════════════════════════════════════════
with tab_soc:
    ...
    if prompt := st.chat_input(
        "Ask the SOC analyst…  e.g. 'Which is the most dangerous IP?'",
        ...
        if ip_counts:
            ...
            with r3l:
                ...
                if fig:
                    ...
                    plt.close(fig)
            with r3r:
                fig = pie_chart(top_ips, "Top IPs — traffic share")
                if fig:
                    st.pyplot(fig, use_container_width=True)
                    plt.close(fig)
        else:
  ...
  <span style="color:#555; font-size:12px;
        ...
        for rel in selected_files:
            lines = read_last_n_lines(
                os.path.join(WATCH_DIR, rel), n_lines)
            if search:
                lines = [l for l in lines if search.lower() in l.lower()]
            with st.expander(
                    f"📄 {rel}  ({len(lines)} lines)", expanded=True):
                if not lines:
...
with tab_flags:
    ...
    if not selected_files or not st.session_state.logging_active:
        ...
        ip_counts  = {}
        for rel in selected_files:
            for line in read_last_n_lines(
                    os.path.join(WATCH_DIR, rel), 1000):
                for ip in ip_pattern.findall(line):
                    ip_counts[ip] = ip_counts.get(ip, 0) + 1
        ip_counts = dict(
            sorted(ip_counts.items(), key=lambda x: x[1], reverse=True))
        ...
        if not ip_counts:
            ...
            for ip, count in list(ip_counts.items())[:30]:
                ...
                col_ip, col_count, col_flag, col_note = st.columns(
                    [2, 1, 1, 4])
                with col_ip:
